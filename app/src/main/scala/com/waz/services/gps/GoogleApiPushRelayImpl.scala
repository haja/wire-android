...
class GoogleApiPushRelayImpl private (context: Context, beConfig: BackendConfig, prefs: GlobalPreferences) extends GoogleApi {

  import GoogleApiPushRelayImpl._
...
object GoogleApiPushRelayImpl {
  val RequestGooglePlayServices = 7976
  val MaxErrorDialogShowCount = 3

  private var instance = Option.empty[GoogleApiPushRelayImpl]

  def apply(context: Context, beConfig: BackendConfig, prefs: GlobalPreferences): GoogleApiPushRelayImpl = synchronized {
    instance match {
      case Some(api) => api
      case None => returning(new GoogleApiPushRelayImpl(context, beConfig, prefs)){ api: GoogleApiPushRelayImpl => instance = Some(api) }
    }
  }
